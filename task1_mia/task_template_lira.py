...
for k, (train_idx, holdout_idx) in enumerate(splits):
    ...
    # Private Scores (Shadow models serve as the "OUT" distribution)
...
# Offline LiRA. So, We only need mu and sigma for "OUT" distribution
params_out = []
for i in range(N):
...
for i in range(N_priv):
    ...
    # Stats for a specific image from shadow models
    mu_out = np.mean(priv_shadow_scores[i])
    sigma_out = np.std(priv_shadow_scores[i])
